...
class CallerFromAlignments:
    ...
    def __chrom_pair_treat_ctrl(self, treat_pv, ctrl_pv) -> list:
        ...
        lt: cython.long
        lc: cython.long
        t_p: cnp.ndarray(cython.int, ndim=1)
        c_p: cnp.ndarray(cython.int, ndim=1)
        ret_p: cnp.ndarray(cython.int, ndim=1)
        t_v: cnp.ndarray(cython.float, ndim=1)
    ...
    def __chrom_call_peak_using_certain_criteria(self,
        ...
        s: str
        above_cutoff: cnp.ndarray
        above_cutoff_endpos: cnp.ndarray(cython.int, ndim=1)
        above_cutoff_startpos: cnp.ndarray(cython.int, ndim=1)
        pos_array: cnp.ndarray(cython.int, ndim=1)
        above_cutoff_index_array: cnp.ndarray(cython.int, ndim=1)
        treat_array: cnp.ndarray(cython.float, ndim=1)
        ctrl_array: cnp.ndarray(cython.float, ndim=1)
    ...
    def __cal_pscore(self,
                     array1: cnp.ndarray(cython.float, ndim=1),
                     array2: cnp.ndarray(cython.float, ndim=1)) -> cnp.ndarray:
    ...
    def __write_bedGraph_for_a_chromosome(self, chrom: bytes) -> bool:
        ...
        """
        pos_array: cnp.ndarray(cython.int, ndim=1)
        treat_array: cnp.ndarray(cython.int, ndim=1)
        ctrl_array: cnp.ndarray(cython.int, ndim=1)
        pos_array_ptr: cython.pointer(cython.int)
        treat_array_ptr: cython.pointer(cython.float)
        ctrl_array_ptr: cython.pointer(cython.float)
        l: cython.int
