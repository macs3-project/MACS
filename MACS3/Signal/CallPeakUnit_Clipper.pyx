...
from MACS3.Signal.Clipper import clipper
# --------------------------------------------
...
cdef class CallerFromAlignments:
    ...
    def __init__ (self, treat, ctrl,
        ...
        # self.ctrl_scaling_factor_s = [1.0, 0.0, 0.0]
    ...
    cdef __pileup_treat_ctrl_a_chromosome ( self, bytes chrom ):
        ...
        # logging.info(f"ctrl_pv is {ctrl_pv}\n")
    ...
    cdef void __cal_pvalue_qvalue_table ( self ):
        ...
        cdef:
            bytes chrom
            np.ndarray pos_array, treat_array, ctrl_array, score_array, clipper_q,regionLength_array
            dict pscore_stat, pscore2qvalue, regionLength_array_dict
            int64_t n, pre_p, length, pre_l, l, i, j, pre_length
            float32_t this_v, pre_v, v, q, pre_q
            int64_t N, k, this_l
            float32_t f, potential_q
            list unique_values
            int32_t * pos_ptr
            int32_t * pos_ptr1
        ...
        for i in range( len( self.chromosomes ) ):
            ...
            # logging.info(f"pos_array shape[0]: \n{str(pos_array.shape[0])}")
            # logging.info(f"pos_array shape[1]: \n{str(pos_array.shape[1])}")
            # logging.info(f"treat_array shape[0]: \n{str(treat_array.shape[0])}")
            # logging.info(f"treat_array shape[1]: \n{str(treat_array.shape[1])}")
            # logging.info(f"ctrl_array shape[0]: \n{str(ctrl_array.shape[0])}")
            # logging.info(f"ctrl_array shape[1]: \n{str(ctrl_array.shape[1])}")


            # f1 = open("../output/pos_array", 'w')
            # f2 = open("../output/treat_array", 'w')
            # f3 = open("../output/ctrl_array", 'w')
            # for j in range(pos_array.shape[0]):
            #     f1.write(f"{<int32_t>(pos_ptr[0])}\n")
            #     f2.write(f"{<int32_t>(treat_value_ptr[0])}\n")
            #     f3.write(f"{(ctrl_value_ptr[0])}\n")
            #     pos_ptr += 1
            #     treat_value_ptr += 1
            #     ctrl_value_ptr += 1
            ...
            # clipper_f = open("/home/wbguo/iproject/Clipper/qvals_2", 'r')
            # lines = clipper_f.readlines()
            # clipper_p = [float(line.strip()) for line in lines]
            # f4 = open("../output/pvals", 'w')
            if debug:

               ################ get the regionLength ############
                # regionLength_array = np.array([])
                # pre_p = 0
                # for j in range(pos_array.shape[0]):
                #    this_l = pos_ptr[0] - pre_p
                #    regionLength_array = np.append(regionLength_array,this_l)
                #    pre_p = pos_ptr[0]
                #    pos_ptr += 1

                regionLength_array_dict = {}
                pre_length = 0
                pos_ptr1 = pos_ptr
                for j in range(pos_array.shape[0]):
                        this_l = pos_ptr1[0] - pre_length
                        regionLength_array_dict[j] = this_l
                        pre_length = pos_ptr1[0]
                        pos_ptr1 += 1

                regionLength_array = np.array(list(regionLength_array_dict.values())).flatten()

                clipper_q = clipper(np.array(treat_array).reshape(1, -1).T, np.array(ctrl_array).reshape(1, -1).T, regionLength=regionLength_array ,analysis="enrichment")['q']
                np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/clipper_q", clipper_q)
                np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/pos_array", np.array(pos_array))
                np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/treat_array", np.array(treat_array))
                np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/ctrl_array", np.array(ctrl_array))

                logging.info("#3 Finished computing qvalue using Clipper")
                pscore2qvalue = {}
            ...
            for j in range(pos_array.shape[0]):
                ...
                if debug:
                    potential_q = clipper_q[ j ]
                    # print(potential_q)
                    if (this_v in pscore2qvalue) and (pscore2qvalue[ this_v ] > potential_q):
                        continue
                    else:
                        pscore2qvalue[ this_v ] = potential_q

            # temp_pqtable = np.array([np.array(temp), clipper_q]).T
            # np.savetxt(f"../output/pqtable_{i}", temp_pqtable)
        ...
        # f1 = open("../output/pvals_unique", 'w')
        # f2 = open("../output/qvals_from_clipper", 'w')
        ...
        for i in range(len(unique_values)):
            ...
            l = pscore_stat[v]
            if debug:
                q = -log10(pscore2qvalue[v])
                clipper_log_q.append(q)
                mac3_q.append(v + (log10(k) + f))
            else:
                q = v + (log10(k) + f)
            # f1.write(f"{<float32_t>(v)}\n")
            ...
            if q <= 0:
                q = 0
                break
            # f2.write(f"{<float32_t>(q)}\n")
            #q = max(0,min(pre_q,q))           # make q-score monotonic
            self.pqtable[ v ] = q
            pre_q = q
            k += l

        np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/clipper_log_q", np.array(clipper_log_q))
        np.savetxt("/home/saidi/Documents/Postdoc/Project_postdoc/Project1_Clipper_AddOn/MAC3/experiment/output/Clipper_added/syn1/debug_output/mac3_q", np.array(mac3_q))
    ...
    cdef void __pre_computes ( self, int32_t max_gap = 50, int32_t min_length = 200 ):
        ...
        cdef:
            ...
            np.ndarray pos_array, treat_array, ctrl_array, score_array, clipper_q, regionLength_array
            dict pscore_stat, pscore2qvalue, regionLength_array_dict
            int64_t n, pre_p, this_p, length, j, pre_l, l, i,pre_length1
            ...
            int32_t * pos_array_ptr1 # position array pointer
        ...
        for i in range( len( self.chromosomes ) ):
            ...
            if debug:
                ...
                regionLength_array_dict = {}
                pre_length1 = 0
                pos_array_ptr1 = pos_array_ptr
                for j in range(pos_array.shape[0]):
                        this_l = pos_array_ptr1[0] - pre_length1
                        regionLength_array_dict[j] = this_l
                        pre_length1 = pos_array_ptr1[0]
                        pos_array_ptr1 += 1
    ...
    cdef void __chrom_call_peak_using_certain_criteria ( self, peaks, bytes chrom, list scoring_function_s, list score_cutoff_s, int32_t min_length,
        ...
        print('scoring_function_s')
        print(scoring_function_s)

        print('score_cutoff_s')
        print(score_cutoff_s)
